...
var DumperApp = function() {
	hxd_App.call(this);
};
$hxClasses["DumperApp"] = DumperApp;
DumperApp.__name__ = ["DumperApp"];
DumperApp.__super__ = hxd_App;
DumperApp.prototype = $extend(hxd_App.prototype,{
	init: function() {
		var hmd = hxd_Res.get_loader().loadModel("Model.FBX").toHmd();
		var dump = new hxd_fmt_hmd_Dump();
		var data = hmd.header;
		data.data = hmd.getData();
		haxe_Log.trace(dump.dump(data),{ fileName : "Dumper.hx", lineNumber : 10, className : "DumperApp", methodName : "init"});
	}
	,__class__: DumperApp
});
var Dumper = function() { };
$hxClasses["Dumper"] = Dumper;
Dumper.__name__ = ["Dumper"];
Dumper.main = function() {
	hxd_Res.set_loader(new hxd_res_Loader(new hxd_fs_EmbedFileSystem(haxe_Unserializer.run("oy8:cube.fbxty8:cube.hmdty9:Model.FBXtg"))));
	new DumperApp();
};
...
var h3d_prim_HMDModel = function() {
...
var hxd_fmt_hmd_Dump = function() {
};
$hxClasses["hxd.fmt.hmd.Dump"] = hxd_fmt_hmd_Dump;
hxd_fmt_hmd_Dump.__name__ = ["hxd","fmt","hmd","Dump"];
hxd_fmt_hmd_Dump.prototype = {
	positionStr: function(p) {
		return "T={" + hxd_Math.fmt(p.x) + "," + hxd_Math.fmt(p.y) + "," + hxd_Math.fmt(p.z) + "} R={" + hxd_Math.fmt(p.qx) + "," + hxd_Math.fmt(p.qy) + "," + hxd_Math.fmt(p.qz) + "} S={" + hxd_Math.fmt(p.sx) + "," + hxd_Math.fmt(p.sy) + "," + hxd_Math.fmt(p.sz) + "}";
	}
	,addProps: function(props) {
		...
		while(_g < props.length) {
			var p = props[_g];
			++_g;
			var params = p.slice(2);
			if(params.length == 0) {
				this.buf.b += Std.string(this.prefix + Std.string(p[0]) + "\n");
			} else {
				var s = p[0] + " : ";
				var _g1 = [];
				var _g2 = 0;
				while(_g2 < params.length) {
					var p1 = params[_g2];
					++_g2;
					_g1.push(Std.string(p1));
				}
				var s1 = s + _g1.join(", ");
				this.buf.b += Std.string(this.prefix + Std.string(s1) + "\n");
			}
		}
	}
	,dump: function(h) {
		...
		this.prefix = "";
		this.buf.b += Std.string(this.prefix + Std.string("HMD v" + h.version) + "\n");
		this.prefix = "\t";
		this.buf.b += Std.string(this.prefix + Std.string("Header : " + hxd_Math.fmt(h.dataPosition / 1024) + " KB") + "\n");
		this.buf.b += Std.string(this.prefix + Std.string("Data : " + hxd_Math.fmt(h.data.length / 1024) + " KB") + "\n");
		this.addProps(h.props);
		this.prefix = "";
		this.buf.b += Std.string(this.prefix + "" + "\n");
		var _g1 = 0;
		var _g = h.geometries.length;
		while(_g1 < _g) {
			var k = _g1++;
			var g = h.geometries[k];
			this.buf.b += Std.string(this.prefix + Std.string("@" + k + " GEOMETRY") + "\n");
			this.prefix += "\t";
			this.buf.b += Std.string(this.prefix + Std.string("Vertex Count : " + g.vertexCount) + "\n");
			this.buf.b += Std.string(this.prefix + Std.string("Vertex Stride : " + g.vertexStride) + "\n");
			var s = "Index Count : " + g.get_indexCount() + " ";
			var s1 = g.indexCounts.length > 1 ? g.indexCounts.toString() : "";
			this.buf.b += Std.string(this.prefix + Std.string(s + s1) + "\n");
			var _this = g.bounds;
			var _this1 = g.bounds;
			this.buf.b += Std.string(this.prefix + Std.string("Bounds : center=" + Std.string(new h3d_col_Point((_this.xMin + _this.xMax) * 0.5,(_this.yMin + _this.yMax) * 0.5,(_this.zMin + _this.zMax) * 0.5)) + " size=" + Std.string(new h3d_col_Point(_this1.xMax - _this1.xMin,_this1.yMax - _this1.yMin,_this1.zMax - _this1.zMin))) + "\n");
			this.buf.b += Std.string(this.prefix + "Format :" + "\n");
			this.addProps(g.props);
			var _g2 = 0;
			var _g3 = g.vertexFormat;
			while(_g2 < _g3.length) {
				var f = _g3[_g2];
				++_g2;
				var s2 = "\t" + f.name + " " + HxOverrides.substr(hxd_fmt_hmd__$Data_GeometryDataFormat_$Impl_$.toString(f.format),1,null);
				this.buf.b += Std.string(this.prefix + Std.string(s2) + "\n");
			}
			this.prefix = "";
		}
		if(h.geometries.length > 0) {
			this.buf.b += Std.string(this.prefix + "" + "\n");
		}
		var flags = hxd_fmt_hmd_MaterialFlag.__empty_constructs__;
		var _g11 = 0;
		var _g4 = h.materials.length;
		while(_g11 < _g4) {
			var k1 = _g11++;
			var m = h.materials[k1];
			this.buf.b += Std.string(this.prefix + Std.string("@" + k1 + " MATERIAL") + "\n");
			this.prefix += "\t";
			if(m.name != null) {
				this.buf.b += Std.string(this.prefix + Std.string("Name : " + m.name) + "\n");
			}
			this.buf.b += Std.string(this.prefix + Std.string("Blend : " + Std.string(m.blendMode)) + "\n");
			this.buf.b += Std.string(this.prefix + Std.string("Cull : " + Std.string(m.culling)) + "\n");
			if(m.diffuseTexture != null) {
				this.buf.b += Std.string(this.prefix + Std.string("Texture : " + m.diffuseTexture) + "\n");
			}
			if(m.killAlpha != null) {
				this.buf.b += Std.string(this.prefix + Std.string("KillAlpha : " + m.killAlpha) + "\n");
			}
			this.addProps(m.props);
			var _g21 = 0;
			while(_g21 < flags.length) {
				var f1 = flags[_g21];
				++_g21;
				if((m.flags & 1 << f1[1]) != 0) {
					this.buf.b += Std.string(this.prefix + Std.string(f1) + "\n");
				}
			}
			this.prefix = "";
		}
		if(h.materials.length > 0) {
			this.buf.b += Std.string(this.prefix + "" + "\n");
		}
		var _g12 = 0;
		var _g5 = h.models.length;
		while(_g12 < _g5) {
			var k2 = _g12++;
			var m1 = h.models[k2];
			this.buf.b += Std.string(this.prefix + Std.string("@" + k2 + " MODEL") + "\n");
			this.prefix += "\t";
			if(m1.name != null) {
				this.buf.b += Std.string(this.prefix + Std.string("Name : " + m1.name) + "\n");
			}
			var p = h.models[m1.parent];
			if(m1.parent >= 0) {
				this.buf.b += Std.string(this.prefix + Std.string("Parent : @" + m1.parent + " " + (p == null ? "INVALID" : p.name == null ? "" : p.name)) + "\n");
			}
			if(m1.follow != null) {
				this.buf.b += Std.string(this.prefix + Std.string("Follow : " + m1.follow) + "\n");
			}
			this.buf.b += Std.string(this.prefix + Std.string("Position : " + this.positionStr(m1.position)) + "\n");
			if(m1.geometry >= 0) {
				this.buf.b += Std.string(this.prefix + Std.string("Geometry : @" + m1.geometry) + "\n");
			}
			if(m1.materials != null) {
				var _g31 = 0;
				var _g22 = m1.materials.length;
				while(_g31 < _g22) {
					var i = _g31++;
					var m2 = m1.materials[i];
					var md = h.materials[m2];
					this.buf.b += Std.string(this.prefix + Std.string("Material " + i + " : @" + m2 + " " + (md == null ? "INVALID" : md.name == null ? "" : md.name)) + "\n");
				}
			}
			this.addProps(m1.props);
			if(m1.skin != null) {
				var s3 = m1.skin;
				this.buf.b += Std.string(this.prefix + "Skin :" + "\n");
				this.prefix += "\t";
				if(s3.name != null) {
					this.buf.b += Std.string(this.prefix + Std.string("Name : " + s3.name) + "\n");
				}
				this.addProps(s3.props);
				var _g32 = 0;
				var _g23 = s3.joints.length;
				while(_g32 < _g23) {
					var i1 = _g32++;
					var j = s3.joints[i1];
					this.buf.b += Std.string(this.prefix + Std.string("@" + i1 + " JOINT") + "\n");
					this.prefix += "\t";
					this.buf.b += Std.string(this.prefix + Std.string("Name : " + j.name) + "\n");
					if(j.parent >= 0) {
						var p1 = s3.joints[j.parent];
						this.buf.b += Std.string(this.prefix + Std.string("Parent : @" + j.parent + " " + (p1 == null ? "INVALID" : p1.name)) + "\n");
					}
					this.buf.b += Std.string(this.prefix + Std.string("Position : " + this.positionStr(j.position)) + "\n");
					if(j.bind >= 0) {
						this.buf.b += Std.string(this.prefix + Std.string("Bind @" + j.bind) + "\n");
					}
					if(j.transpos != null) {
						this.buf.b += Std.string(this.prefix + Std.string("TransPos : " + this.positionStr(j.transpos)) + "\n");
					}
					this.addProps(j.props);
					this.prefix = HxOverrides.substr(this.prefix,1,null);
				}
				if(s3.split != null) {
					var s4 = "Split : " + s3.split.length + " " + s3.split.toString();
					this.buf.b += Std.string(this.prefix + Std.string(s4) + "\n");
				}
			}
			this.prefix = "";
		}
		...
		var FLAGS = hxd_fmt_hmd_AnimationFlag.__empty_constructs__;
		var _g13 = 0;
		var _g6 = h.animations.length;
		while(_g13 < _g6) {
			var k3 = _g13++;
			var a = h.animations[k3];
			this.buf.b += Std.string(this.prefix + Std.string("@" + k3 + " ANIMATION") + "\n");
			this.prefix += "\t";
			if(a.name != null) {
				this.buf.b += Std.string(this.prefix + Std.string("Name : " + a.name) + "\n");
			}
			this.buf.b += Std.string(this.prefix + Std.string("Frames : " + a.frames) + "\n");
			this.buf.b += Std.string(this.prefix + Std.string("Sampling : " + a.sampling) + "\n");
			this.buf.b += Std.string(this.prefix + Std.string("Speed : " + a.speed) + "\n");
			this.buf.b += Std.string(this.prefix + Std.string("Loop : " + (a.loop == null ? "null" : "" + a.loop)) + "\n");
			this.addProps(a.props);
			this.buf.b += Std.string(this.prefix + "Objects : " + "\n");
			this.prefix += "\t";
			var _g24 = 0;
			var _g33 = a.objects;
			while(_g24 < _g33.length) {
				...
				var flags1 = [];
				var _g41 = 0;
				while(_g41 < FLAGS.length) {
					var f2 = FLAGS[_g41];
					++_g41;
					if((o.flags & 1 << f2[1]) != 0) {
						var n = f2[0];
						if(StringTools.startsWith(n,"Has")) {
							n = HxOverrides.substr(n,3,null);
						}
						if(f2 == hxd_fmt_hmd_AnimationFlag.HasProps) {
							n += Std.string(o.props);
						}
						flags1.push(n);
					}
				}
				var s5 = "" + o.name + " : " + flags1.join(",");
				this.buf.b += Std.string(this.prefix + Std.string(s5) + "\n");
			}
			this.prefix = HxOverrides.substr(this.prefix,1,null);
			if(a.events != null) {
				this.buf.b += Std.string(this.prefix + "Events : " + "\n");
				this.prefix += "\t";
				var _g25 = 0;
				var _g34 = a.events;
				while(_g25 < _g34.length) {
					var e = _g34[_g25];
					++_g25;
					this.buf.b += Std.string(this.prefix + Std.string("" + e.frame + " : " + e.data) + "\n");
				}
				this.prefix = HxOverrides.substr(this.prefix,1,null);
			}
			this.prefix = "";
		...
		var d = new haxe_io_BytesInput(h.data);
		var _g14 = 0;
		var _g7 = h.geometries.length;
		while(_g14 < _g7) {
			var k4 = _g14++;
			var g1 = h.geometries[k4];
			var stride = 0;
			var _g26 = 0;
			var _g35 = g1.vertexFormat;
			while(_g26 < _g35.length) {
				var f3 = _g35[_g26];
				++_g26;
				stride += f3.format & 7;
			}
			this.buf.b += Std.string(this.prefix + Std.string("@" + k4 + " GEOMETRY") + "\n");
			this.prefix += "\t";
			d.set_position(g1.vertexPosition);
			var _g27 = [];
			var _g42 = 0;
			var _g36 = g1.vertexCount;
			while(_g42 < _g36) {
				_g42++;
				var _g51 = [];
				var _g71 = 0;
				...
				while(_g71 < _g61) {
					_g71++;
					_g51.push(d.readFloat());
				}
				_g27.push(_g51);
			}
			this.buf.b += Std.string(this.prefix + Std.string("VertexBuffer : " + Std.string(_g27)) + "\n");
			d.set_position(g1.indexPosition);
			var _g37 = [];
			var _g52 = 0;
			var _g43 = g1.get_indexCount();
			while(_g52 < _g43) {
				_g52++;
				_g37.push(d.readUInt16());
			}
			this.buf.b += Std.string(this.prefix + Std.string("IndexBuffer : " + Std.string(_g37)) + "\n");
			this.prefix = "";
		}
		if(h.geometries.length > 0) {
			this.buf.b += Std.string(this.prefix + "" + "\n");
		...
		var _g15 = 0;
		var _g8 = h.animations.length;
		while(_g15 < _g8) {
			var k5 = _g15++;
			var a1 = h.animations[k5];
			this.buf.b += Std.string(this.prefix + Std.string("@" + k5 + " ANIMATION") + "\n");
			this.prefix += "\t";
			d.set_position(a1.dataPosition);
			var _g28 = 0;
			var _g38 = a1.objects;
			while(_g28 < _g38.length) {
				...
				var stride1 = 0;
				if((o1.flags & 1 << hxd_fmt_hmd_AnimationFlag.HasPosition[1]) != 0) {
					stride1 += 3;
				}
				if((o1.flags & 1 << hxd_fmt_hmd_AnimationFlag.HasRotation[1]) != 0) {
					stride1 += 3;
				}
				if((o1.flags & 1 << hxd_fmt_hmd_AnimationFlag.HasScale[1]) != 0) {
					stride1 += 3;
				}
				if((o1.flags & 1 << hxd_fmt_hmd_AnimationFlag.SinglePosition[1]) != 0) {
					frames = 1;
				}
				if(stride1 > 0) {
					var s6 = "" + o1.name + " Position : ";
					var _g44 = [];
					var _g62 = 0;
					var _g53 = frames;
					while(_g62 < _g53) {
						...
						while(_g9 < _g81) {
							_g9++;
							_g72.push(d.readFloat());
						}
						_g44.push(_g72);
					}
					this.buf.b += Std.string(this.prefix + Std.string(s6 + Std.string(_g44)) + "\n");
				}
				if((o1.flags & 1 << hxd_fmt_hmd_AnimationFlag.HasUV[1]) != 0) {
					var s7 = "" + o1.name + " UV : ";
					var _g45 = [];
					var _g63 = 0;
					var _g54 = a1.frames;
					while(_g63 < _g54) {
						...
						while(_g82 < 2) {
							_g82++;
							_g73.push(d.readFloat());
						}
						_g45.push(_g73);
					}
					this.buf.b += Std.string(this.prefix + Std.string(s7 + Std.string(_g45)) + "\n");
				}
				if((o1.flags & 1 << hxd_fmt_hmd_AnimationFlag.HasAlpha[1]) != 0) {
					var s8 = "" + o1.name + " Alpha : ";
					var _g46 = [];
					var _g64 = 0;
					var _g55 = a1.frames;
					while(_g64 < _g55) {
						_g64++;
						_g46.push(d.readFloat());
					}
					this.buf.b += Std.string(this.prefix + Std.string(s8 + Std.string(_g46)) + "\n");
				}
				if((o1.flags & 1 << hxd_fmt_hmd_AnimationFlag.HasProps[1]) != 0) {
					var _g47 = 0;
					var _g56 = o1.props;
					while(_g47 < _g56.length) {
						var p2 = _g56[_g47];
						++_g47;
						var s9 = "" + o1.name + " " + p2 + " : ";
						var _g65 = [];
						...
						while(_g83 < _g74) {
							_g83++;
							_g65.push(d.readFloat());
						}
						this.buf.b += Std.string(this.prefix + Std.string(s9 + Std.string(_g65)) + "\n");
					}
				}
			}
			this.prefix = "";
		...
		this.buf.b += Std.string(this.prefix + "DONE" + "\n");
		return this.buf.b;
	}
	,__class__: hxd_fmt_hmd_Dump
};
